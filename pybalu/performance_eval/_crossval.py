...
def crossval(features, classification, classifier, n_folds, *, confidence=.95, classifier_opts=None, strat=False):
    ...
    if N != classification.size:
        raise ValueError(f'Dim 0 of `features` must be equal to `classification` size. ({N} != {classification.size})')
    ...
    if strat and any(counts < n_folds):
        raise ValueError('Some classes have less occurences than the number of folds!')
    ...
    if strat:
        ...
        idx_vec = np.hstack([np.arange(s, s+v) for s,v in zip(starts, cls_size)])
        diff = np.repeat(cls_size, cls_size)
        starts += cls_size * (n_folds - 1)
        first_blocks = [idx_vec + diff*v for v in range(n_folds-1)] 
        last_block = [np.hstack([np.arange(s, e) for s, e in zip(starts, ends)])]
        blocks = first_blocks + last_block
    else:
        block_size = N // n_folds
        first_blocks = [np.arange(block_size*i, block_size*(i+1)) for i in range(n_folds-1)] 
