__all__ = ['hog', 'HOGExtractor']

import numpy as np
from scipy.signal import convolve2d
from pybalu.misc import im2row

from ._hog_utils_c import sep_into_bins
from ._feature_extractor import FeatureExtractorBase

_hj = np.array([[1, 0, -1]])
_hi = - _hj.T

def hog(image, region=None, *, v_windows=0, h_windows=0, n_bins=0, normalize=False, labels=False, show=False):
    '''\
hog(image, region=None, *, v_windows=0, h_windows=0, n_bins=0, normalize=False, labels=False, show=False)
...
'''
    if not isinstance(v_windows, int) or v_windows <= 0:
        raise ValueError(f"`v_windows` must be an integer value greater than 0")
    if not isinstance(h_windows, int) or h_windows <= 0:
        raise ValueError(f"`h_windows` must be an integer value greater than 0")
...
class HOGExtractor(FeatureExtractorBase):
    extractor_func = hog